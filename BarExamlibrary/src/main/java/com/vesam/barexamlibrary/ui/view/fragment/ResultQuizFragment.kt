...
class ResultQuizFragment : Fragment() {
    ...
    private fun initPopBackStack() {
        ...
        when {
            resultQuizTag.equals(RESULT_QUIZ_DETAIL_BUNDLE) -> navController.popBackStack()
            else -> {
                navController.popBackStack()
                navController.popBackStack()
            }
    ...
    private fun initSetView(it: ResponseSetQuizQuestionModel) {
        binding.progressBarPercentToPassed.setProgress(it.quiz.result.percentToPassed)
        binding.txtPercentToPassed.text = "% ${it.quiz.result.percentToPassed}"
        binding.txtTitle.text = it.quiz.title
